...
class Hadamard_Rand_1bit:
    def __init__(self, absz, pri_para, encode_acc = 0): # absz: alphabet size, pri_para: privacy parameter
        ...
        self.flip_prob = 1/(1+math.exp(pri_para)) #flipping probability to maintain local privacy
        ...
        if encode_acc == 1:
            self.H = Hadamard_init(self.outsz) # initialize Hadamard matrix
    ...
    def encode_string(self,in_list):
        ...
        out_list = np.zeros(l)
        for i in range(0,l):
            x = self.H[i%self.outsz][in_list[i] + 1]
            ra = random.random()
            if ra < self.flip_prob: # flip output bit with prob self.flip_prob
                out_list[i] = 1 - x
            else: 
                out_list[i] = x
        return out_list

    def decode_string(self, out_list, normalization = 0): # get the privatized string and learn the original distribution
        #normalization options: 0: clip and normalize(default)
        #                       1: simplex projection
        #                       else: no nomalization
        p_Y = np.zeros(self.outsz)
        l = len(out_list)
        for i in range(0,l):
            p_Y[i%self.outsz] = p_Y[i%self.outsz] + out_list[i]
        p_Y = p_Y/l * self.outsz
        p_C = (p_Y - self.flip_prob)/(self.exp-1)*(self.exp+1)
        p_K = FWHT_A(self.outsz, 2*p_C - np.ones(self.outsz))/self.outsz
        dist = p_K[0:self.insz]
...
class Hadamard_Rand_general:
    def __init__(self, absz, pri_para, encode_acc = 0): # absz: alphabet size, pri_para: privacy parameter
        ...
        self.ifencode_acc = encode_acc #whether to accelerate encoding process
        if encode_acc == 1:
            self.H = Hadamard_init(self.partsz) # initialize Hadarmard matrix
...
#the Hadamard randamized responce when \epsilon < 1
class Hadamard_Rand_1bit_new:
    def __init__(self, absz, pri_para, encode_acc = 0): # absz: alphabet size, pri_para: privacy parameter
        #set encode_acc = 1 to enable fast encoding by intializing hadamard matrix
        self.insz = absz #input alphabet size k
        self.outsz = int(math.pow(2,math.ceil(math.log(absz+1,2)))) * 64#output alphabet size: smallest exponent of 2 which is bigger than k
        self.outbit = int(math.ceil(math.log(absz+1,2))) + 6#output bit length
        self.exp = math.exp(pri_para) #privacy parameter
        self.flip_prob = 1/(1+math.exp(pri_para)) #flipping probability to maintain local privacy
        #self.permute, self.reverse = Random_Permutation(absz) #Initialize random permutation
        self.ifencode_acc = encode_acc #whether to accelerate encoding process
...
# Testing Script for running time
if __name__ == "__main__":
    ...
    a = Hadamard_Rand_1bit_new(k,eps,1) # define the private channel
    print(a.outsz)
    t1 = 0
    t2 = 0
...
#        t2 = t2 + timeit.default_timer() - start_time
    ...
    a = Hadamard_Rand_1bit(k,eps,1) # define the private channel
    print(a.outsz)
    t1 = 0
    t2 = 0
    outp = a.encode_string(in_list) # get the output string
...
if __name__ == "__main__":
    ...
    prob = [1/float(k)] * k
    in_list = np.random.choice(elements, 100000, p=prob) #input symbols
    a = Hadamard_Rand_high_priv(100,eps,1) # define the private channel
    t1 = 0
    t2 = 0
    for i in range(1,51):
        print('Iteration',i)
